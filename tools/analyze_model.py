...
def add_modalities_to_data(cfg, data):
    import torch
    ...
    for modality in cfg.DATASETS.MODALITIES.ORDER:
        ...
        if modality_key != main_modality:
            if "muses" in cfg.INPUT.DATASET_MAPPER_NAME.lower():
                ...
                modalities.append(modality_key)
    data[0]["modalities"] = modalities
    for modality in modalities:
        data[0][modality] = torch.zeros((3, crop_size[0], crop_size[1]))
    return data
...
def do_flop(cfg):
    ...
    for idx, data in zip(tqdm.trange(1), data_loader):  # noqa
        if args.use_fixed_input_size and isinstance(cfg, CfgNode):
            ...
            data = add_modalities_to_data(cfg, data)
    ...
    total_flops = np.mean(total_flops) / 1e9
    logger.info(f"Total GFlops: {total_flops:.1f}   ±{np.std(total_flops) / 1e9:.1f}\n")
...
def do_parameter(cfg):
    ...
    logger.info(f"Total Params: {total_params:.1f} M\n")
